...
namespace ePacker1
{
    ...
    {
        string session, strSQL, Group_ID, Challan_No, Invoice_No;
        ...
        // Below 2 tables are to be considered as 1 for saving and fetching Invoice data
        //select* from Dispatch_InvCopy
        //select* from Dispatch_Challan
        // Exe below query on system for data consistency
        // DELETE from Dispatch_Challan where Challan_No not in ( select Challan_No from Dispatch_InvCopy)
        private void RPeP_DispNewChallan_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                //Challan_No = funclib.Challan_ID("I", con, "select InvChallan_No from tbl_DispatchInvoiceChallan order by InvChallan_No  desc");
                //txtChallan_No.Text = Challan_No;

                Invoice_No = funclib.I_ID("I", con, "select InvChallan_No  from tbl_DispatchInvoiceChallan order by InvChallan_No  desc");
                txtInv_No.Text = Invoice_No;
                txtChallan_No.Text = Invoice_No;


                SqlDataAdapter da = new SqlDataAdapter("SELECT Order_ID from Item_Order as a,Party_Master as b where a.P_ID=b.P_ID  and a.Grp_ID = '" + Group_ID + "' and a.Order_Active='Yes' ", con);
            ...
            {
                MessageBox.Show("There was an error while Loading Invoice  Form" + ex.Message);
            }
        }
        ...
        private void ddlOrder_ID_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                //select* from Dispatch_Challan DC
                //INNER JOIN  Dispatch_InvCopy DIC
                //ON DC.Order_ID = DIC.Order_ID
                //INNER JOIN Item_Order IO
                //ON IO.Order_ID = DIC.Order_ID
            ...
            try
            {
                ...
                if (sfd.ShowDialog() != DialogResult.Cancel)
                {
                    using (ExcelPackage pck = new ExcelPackage())
                    {
                        ...
                        ExcelWorksheet ws = pck.Workbook.Worksheets.Add("InvoiceMaster");
        ...
        private void cmdSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    SqlCommand cmd3 = new SqlCommand("select Desp_Det  from tbl_DispatchInvoiceChallan where Order_ID ='" + ddlOrder_ID.SelectedValue.ToString() + "'", con1);
                    con1.Open();
                    SqlDataReader dr1 = cmd3.ExecuteReader();
                    if (dr1.Read())
                    {
            ...
            {
                MessageBox.Show("There was an error while Adding Invoice " + ex.Message);
            }
        }
        ...
        protected void ClearAll()
        {
            try
            {
                ...
                //Challan_No = funclib.I_ID("CH", con, "select InvChallan_No from tbl_DispatchInvoiceChallan order by InvChallan_No  desc");
                //txtChallan_No.Text = Challan_No;
            ...
            {
                MessageBox.Show("There was an error while Clearing Form " + ex.Message);
            }
        }
        ...
        private void CmdSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("There was an error while Searching Invoice " + ex.Message);
            }
        }
        ...
        private void GridView1_RowHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            try
            {
                ...
                SqlCommand cmd3 = new SqlCommand("select convert (varchar(20),a.Inv_Dt,103)  as 'Inv_Dt',LR_No,Veh_No,Inv_Type,Local_Sales,Fre_Amt,Exc_Amt,Bundle_Note,Per_Det,Desp_Det,Wt_Det,AvgWg_Det,Rate_Det,Disc_Det,Total_Amt,Discount_Amt,Net_Amt,Excise_Amt,Edu_Cess_Amt,HSEC_Amt,CST_VAT_Amt,Freight_Amt,Grand_Amt,Inv_Active from tbl_DispatchInvoiceChallan as a where a.InvChallan_No ='" + txtChallan_No.Text + "' ", con1);
                con1.Open();
                SqlDataReader dr1 = cmd3.ExecuteReader();
                if (dr1.Read())
                {

                    txtChallan_Dt.Text = Convert.ToString(dr1["Inv_Dt"]);
                    txtLR_No.Text = Convert.ToString(dr1["LR_No"]);
                    txtVeh_No.Text = Convert.ToString(dr1["Veh_No"]);
                    txtChallan_Type.Text = Convert.ToString(dr1["Inv_Type"]);
                    ...
                    ddlActive.Text = Convert.ToString(dr1["Inv_Active"]);
            ...
            {
                MessageBox.Show("There was an error while Loading Invoice Data upon delection" + ex.Message);
            }
        }
        ...
        private void cmdEdit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (MessageBox.Show("Do you wish to Update this records", "Confirmation", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    ...
                    SqlConnection con = new SqlConnection(strcon);
                    con.Open();
                    SqlCommand cmd = new SqlCommand("update tbl_DispatchInvoiceChallan set Challan_Active  ='" + ddlActive.Text + "',Mod_By='" + session + "',Mod_ByNode='',Mod_On= convert(datetime,getdate(),103) where InvChallan_No ='" + txtChallan_No.Text.Trim().Replace("'", "''").ToString() + "'", con);
            ...
            {
                MessageBox.Show("There was an error while Editing Invoice " + ex.Message);
            }                   
        }
        ...
        protected void pdfGenerate()
        {
            ...
            {
                MessageBox.Show("There was an error while Creating Invoice PDF" + ex.Message);
            }
        }
        ...
        private void cmdPrint_Click(object sender, EventArgs e)
        {
            ...
            if (sfd.ShowDialog() == DialogResult.OK)
            {
                ...
                {
                    //Handle document exception
                    MessageBox.Show("There was an DocumentException while Printing Invoice PDF" + dex.Message);
                }
                catch (IOException ioex)
                {
                    //Handle IO exception
                    MessageBox.Show("There was an IO while Printing Invoice PDF" + ioex.Message);
                }
                catch (Exception ex)
                {
                    //Handle Other Exception
                    MessageBox.Show("There was an Other while Printing Invoice PDF" + ex.Message);
                }
                finally
                {
                    doc.Close(); //Close document
