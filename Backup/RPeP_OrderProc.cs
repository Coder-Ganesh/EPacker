...
namespace ePacker1
{
    public partial class RPeP_OrderProc : Form
    {
        ...
        private void ddlI_ID_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (ddlI_ID.SelectedValue != null)
            {
                ...
                SqlCommand cmd3 = new SqlCommand("select EF_NF_PC,Sell_Rate,Box_PerSheet from Item_Master where I_ID='" + ddlI_ID.SelectedValue.ToString() + "'", con1);
                con1.Open();
        ...
        {
            if (txtOrder_ID.Text == "")//checking for PartyName  field 
            {
                MessageBox.Show("Order ID Cannot be Blank");
                txtOrder_ID.Focus();
            }

            else if (txtOrder_Dt.Text == "")//checking for blank name text field 
            {
                MessageBox.Show("Field Cannot Be Balnk");
                txtOrder_Dt.Focus();
            }
            else if (ddlP_ID.Text == "")//checking for blank Phone
            {
                MessageBox.Show("Field Cannot Be Balnk");
                ddlP_ID.Focus();
        ...
        private void CmdSearch_Click(object sender, EventArgs e)
        {
            ...
            else if (ddlSearch.Text == "Item")
            {
                ...
                string strcon = funclib.setConnection();
                SqlConnection con = new SqlConnection(strcon);
                SqlDataAdapter da = new SqlDataAdapter(" select a.Order_ID as 'Order ID',convert (varchar(20),a.Order_Dt,103)  as 'Order Dt.',b.P_Name as 'Party',a.P_PONo  as 'Party PO No.',convert (varchar(20),a.P_PODt,103)  as 'Party PO Dt.',c.I_Name as 'Item',a.Box_Qty  as 'Box Qty.',a.Sell_Rate  as 'Selling Rate',a.SubItem_Of  as 'Sub Order Of',a.P_ID,a.I_ID,a.Box_PerSheet,a.Sheet_Qty,a.Print_Inst,a.Order_Active   from Item_Order as a,Party_Master as b,Item_Master as c where a.P_ID=b.P_ID and a.I_ID=c.I_ID and c.Item like  '%" + txt_Search.Text + "%'  order by a.Order_ID", con);
                DataTable dt = new DataTable();
                da.Fill(dt);
                GridView1.DataSource = dt;
