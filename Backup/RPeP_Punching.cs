...
namespace ePacker1
{
    public partial class RPeP_Punching : Form
    {
        ...
        private void RPeP_Punching_Load(object sender, EventArgs e)
        {
            ...
            SR_ID = funclib.PSPI_ID("PSPU", con, "select PSPU_ID  from Punching_Details order by PSPU_ID desc");
        ...
        private void cmdOrderSearch_Click(object sender, EventArgs e)
        {
            ...
            SqlDataAdapter da = new SqlDataAdapter("SELECT '' as Order_ID UNION select Order_ID from Item_Order where I_ID in (select I_ID from Item_Master where Punching_Opt='Yes') and Order_ID not in (select Order_ID from Punching_Details) and Order_Active='Yes' and Grp_ID='" + Group_ID + "' and Order_ID like '%" + txtorderSearch.Text + "%'", con);
            DataTable dt = new DataTable();
            da.Fill(dt);
            con.Open();
        ...
        private void cmdSubmit_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                SqlCommand cmdPositive = new SqlCommand("insert into Punching_Details(PSPU_ID,Grp_ID,PSPU_Dt,Order_ID,M_ID,PSPU_BoxQty,PSPU_NoPins,PSPU_Mfg,S_ID,PSPU_ContrTime,Add_By,Add_ByNode,Add_On) values('" + txtPSSC_ID.Text.Trim().Replace("'", "''").ToString() + "','" + Group_ID + "',convert(datetime,'" + txtPSSC_Dt.Text.Trim().Replace("'", "''").ToString() + "',103),'" + ddlOrder_ID.SelectedValue.ToString() + "','" + ddlM_ID.SelectedValue.ToString() + "','" + txtPSSC_BoxQty.Text.Trim().Replace("'", "''").ToString() + "','" + txtPSSC_NoPins.Text.Trim().Replace("'", "''").ToString() + "','" + txtPSSC_Mfg.Text.Trim().Replace("'", "''").ToString() + "','" + ddlS_ID.SelectedValue.ToString() + "','" + txtPSSC_ContrTime.Text.Trim().Replace("'", "''").ToString() + "','" + session + "','',convert(datetime,getdate(),103))", con);
                con.Open();
        ...
        private void cmdEdit_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("Do you wish to Update this records", "Confirmation", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                SqlConnection con = new SqlConnection(strcon);
                con.Open();
                SqlCommand cmd = new SqlCommand("update Punching_Details set M_ID ='" + ddlM_ID.SelectedValue.ToString() + "',PSPU_BoxQty = '" + txtPSSC_BoxQty.Text.Trim().Replace("'", "''").ToString() + "',PSPU_NoPins = '" + txtPSSC_NoPins.Text.Trim().Replace("'", "''").ToString() + "',PSPU_Mfg = '" + txtPSSC_Mfg.Text.Trim().Replace("'", "''").ToString() + "',S_ID = '" + ddlS_ID.SelectedValue.ToString() + "',PSPU_ContrTime = '" + txtPSSC_ContrTime.Text.Trim().Replace("'", "''").ToString() + "',Mod_By='" + session + "',Mod_ByNode='',Mod_On= convert(datetime,getdate(),103) where PSPU_ID  ='" + txtPSSC_ID.Text.Trim().Replace("'", "''").ToString() + "'", con);
        ...
        private void CmdSearch_Click(object sender, EventArgs e)
        {
            ...
            string strcon = funclib.setConnection();
            SqlConnection con = new SqlConnection(strcon);
            SqlDataAdapter da = new SqlDataAdapter(" select PSPU_ID,convert (varchar(20),PSPU_Dt,103) as 'Date',Order_ID as 'Order No',a.M_ID,b.M_Name as 'Machine',PSPU_BoxQty as 'Box Qty',PSPU_NoPins as 'No of Pins',PSPU_Mfg as 'Manufacturing',a.S_ID,c.S_Name as 'Shift',PSPU_ContrTime as 'Contact time' from Punching_Details as a,Machine_Master as b,Shift_Master as c where a.M_ID=b.M_ID and a.S_ID=c.S_ID and a.Order_ID like '%" + txt_Search.Text.ToString() + "%' ", con);
            DataTable dt = new DataTable();
            da.Fill(dt);
