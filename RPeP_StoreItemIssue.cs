...
namespace ePacker1
{
    public partial class RPeP_StoreItemIssue : Form
    {
        string session, strSQL, Group_ID, SR_ID, SubOrder, SubItem_Of;
        private FunctionLib funclib;
        public RPeP_StoreItemIssue()
        {
            InitializeComponent();
        ...
        private void RPeP_StoreItemIssue_Load(object sender, EventArgs e)
        {
            ...
            SR_ID = funclib.PSPI_ID("PSST", con, "select PSST_ID  from StoreItemIssue_Details order by PSST_ID desc");
        ...
        private void cmdOrderSearch_Click(object sender, EventArgs e)
        {
            ...
            SqlDataAdapter da = new SqlDataAdapter("SELECT '' as Order_ID UNION select Order_ID from Item_Order where I_ID in (select I_ID from Item_Master) and Order_ID not in (select Order_ID from StoreItemIssue_Details) and Order_Active='Yes' and Grp_ID='" + Group_ID + "' and Order_ID like '%" + txtorderSearch.Text + "%'", con);
        ...
        private void txtPSST_Qty_Leave(object sender, EventArgs e)
        {
            //if (txtPSST_Qty.Text == "")
            //{
            //    txtPSST_Qty.Text = "0";
        ...
        private void cmdSubmit_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Outword No Cannot be Blank");
                txtPSST_ID.Focus();
            }
            ...
            else if (txtPSST_IDesc.Text == "")//checking for blank Phone
            {
                MessageBox.Show("Field Cannot Be Balnk");
                txtPSST_IDesc.Focus();
            ...
            else
            {
                ...
                SqlCommand cmdPositive = new SqlCommand("insert into StoreItemIssue_Details(PSST_ID,Grp_ID,PSST_Dt,Order_ID,I_ID,PSST_IDesc,PSST_Qty,Add_By,Add_ByNode,Add_On) values('" + txtPSST_ID.Text.Trim().Replace("'", "''").ToString() + "','" + Group_ID + "',convert(datetime,'" + txtPSST_Dt.Text.Trim().Replace("'", "''").ToString() + "',103),'" + ddlOrder_ID.SelectedValue.ToString() + "','" + ddlI_ID.SelectedValue.ToString() + "','" + txtPSST_IDesc.Text.Trim().Replace("'", "''").ToString() + "','" + txtPSST_Qty.Text.Trim().Replace("'", "''").ToString() + "','" + session + "','',convert(datetime,getdate(),103))", con);
                con.Open();
        ...
        private void cmdEdit_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("Do you wish to Update this records", "Confirmation", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                SqlConnection con = new SqlConnection(strcon);
                con.Open();
                SqlCommand cmd = new SqlCommand("update StoreItemIssue_Details set PSST_IDesc = '" + txtPSST_IDesc.Text.Trim().Replace("'", "''").ToString() + "',Mod_By='" + session + "',Mod_ByNode='',Mod_On= convert(datetime,getdate(),103) where PSST_ID  ='" + txtPSST_ID.Text.Trim().Replace("'", "''").ToString() + "'", con);
        ...
        private void CmdSearch_Click(object sender, EventArgs e)
        {
            ...
            string strcon = funclib.setConnection();
            SqlConnection con = new SqlConnection(strcon);
            SqlDataAdapter da = new SqlDataAdapter(" select PSST_ID,convert (varchar(20),PSST_Dt,103) as 'Date',Order_ID as 'Order No',a.I_ID,b.I_Name as 'Item',PSST_IDesc as 'Description',PSST_Qty as 'Quantity' from StoreItemIssue_Details as a,Item_Master as b where a.I_ID=b.I_ID and a.Order_ID like '%" + txt_Search.Text.ToString() + "%' ", con);
            DataTable dt = new DataTable();
            da.Fill(dt);
        ...
        private void GridView1_RowHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            txtPSST_IDesc.Text = GridView1.Rows[e.RowIndex].Cells[5].Value.ToString();
            ...
            txtPSST_Qty.Text = GridView1.Rows[e.RowIndex].Cells[6].Value.ToString();
